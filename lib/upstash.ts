...
export async function setRandomKey(
  ...
  if (response !== "OK") {
    ...
    pipeline.zadd(`acme.st:clicks:${key}`, {
      score: Date.now(),
      member: {
        geo: LOCALHOST_GEO_DATA,
        ua: "Acmest-Bot",
        referer: "https://acme.st",
        timestamp: Date.now(),
      },
    });
    pipeline.expire(`acme.st:clicks:${key}`, 30 * 60); // 30 minutes
    await pipeline.exec();
    return { response, key };
  }
}
